...
uint8_t NRF24_read_register(uint8_t reg)
{
	...
	//while (HAL_SPI_GetState(&nrf24_hspi) != HAL_SPI_STATE_READY);

	//Receive data
	HAL_SPI_Receive(&nrf24_hspi, &spiBuf[1], 1, 100);
	//HAL_SPI_Receive_DMA(&nrf24_hspi, buf, 1);
...
uint8_t NRF24_read_register(uint8_t reg)
{
    uint8_t spiBuf[2]; // Solo necesitamos 2 bytes para transmitir y recibir un byte
    uint8_t retData;

    NRF24_csn(0);
    spiBuf[0] = reg & 0x1F;

    HAL_SPI_TransmitReceive_DMA(&nrf24_hspi, spiBuf, spiBuf, 2);

    // Esperar a que la transferencia DMA se complete (opcional, depende de tu aplicación)
    //while (HAL_SPI_GetState(&nrf24_hspi) != HAL_SPI_STATE_READY);
...
uint8_t NRF24_read_register(uint8_t reg)
{
	...
	// Transmit register address
	txData[0] = reg & 0x1F;
	HAL_SPI_Transmit_DMA(&nrf24_hspi, txData, 1);

	// Wait for DMA to complete
	while (HAL_SPI_GetState(&nrf24_hspi) != HAL_SPI_STATE_READY);

	// Receive data
	HAL_SPI_Receive(&nrf24_hspi, rxData, 1, 100);
...
void NRF24_write_payload(const void* buf, uint8_t len)
{
	...
	//HAL_SPI_Transmit_DMA(&nrf24_hspi, &wrPayloadCmd, 1);
	HAL_SPI_Transmit(&nrf24_hspi, (uint8_t *)buf, len, 100);
	//HAL_SPI_Transmit_DMA(&nrf24_hspi, (uint8_t *)buf, len);
...
{
	uint8_t cmdRxBuf; 		//Get data length using payload size
	uint8_t data_len = MIN(len, NRF24_getPayloadSize());
	NRF24_csn(0);			//Read data from Rx payload buffer
	cmdRxBuf = CMD_R_RX_PAYLOAD;


	//HAL_SPI_Transmit(&nrf24_hspi, &cmdRxBuf, 1, 100);
	HAL_SPI_Transmit_DMA(&nrf24_hspi, &cmdRxBuf, 1);

	HAL_SPI_Receive(&nrf24_hspi, buf, data_len, 100);
	//HAL_SPI_Receive_DMA(&nrf24_hspi, buf, data_len);
...
void NRF24_begin(GPIO_TypeDef *nrf24PORT, uint16_t nrfCSN_Pin, uint16_t nrfCE_Pin, SPI_HandleTypeDef nrfSPI)
{
	...
	NRF24_setRetries(5, 15); //15, 15
	//Initialize PA level to max (0dB)
	NRF24_setPALevel(RF24_PA_LOW); //NRF24_setPALevel(RF24_PA_0dB);
	//Initialize data rate to 1Mbps
	NRF24_setDataRate(RF24_1MBPS); //RF24_2MBPS
...
{
	uint8_t pipe = 0;
	return NRF24_availablePipe(&pipe);
}
...
void NRF24_openWritingPipe_B(uint8_t* address)
{
...
//19. Open reading pipe
void NRF24_openReadingPipe_B(uint8_t number, uint8_t* address){
	if (number == 0){
		memcpy(pipe0_reading_address, address, 5);
		//_is_p0_rx = true;
	}
	if(number <= 5){
		if(number < 2){
			//Address width is 5 bytes
			NRF24_write_registerN(NRF24_ADDR_REGS[number], address, 5);
		}
		else{
			NRF24_write_registerN(NRF24_ADDR_REGS[number], address, 1);
		}
		//Write payload size
		NRF24_write_register(RF24_RX_PW_PIPE[number],payload_size);
		//Enable pipe
...
{
	const uint8_t max_channel = 125; //127
...
bool NRF24_availablePipe(uint8_t* pipe_num)
{
  ...
  if (result)
  {
    ...
    if ( pipe_num )
      *pipe_num = ( status >> BIT_RX_P_NO ) & 0x07; //0x7
...
{
  uint8_t status = NRF24_get_status();
  uint8_t pipe = (status >> BIT_RX_P_NO) & 0x07;
...
void NRF24_ACTIVATE_cmd(void)
{
	...
	HAL_SPI_Transmit_DMA(&nrf24_hspi, cmdRxBuf, 2);
	NRF24_csn(1);
}
